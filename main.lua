...
local cachedAnimalListFromMemory = nil
local profileReady = false

local function animalListLooksValid(al)
	if type(al) ~= "table" then return false end
	local n, withIndex = 0, 0
	for _, d in pairs(al) do
		n += 1
		if type(d) == "table" and d.Index then withIndex += 1 end
		...
		pcall(function()
			for _, v in pairs(getgc(true)) do
				...
				if type(v) == "table" then
					local okBI, bi = pcall(rawget, v, "BaseSkinInventory")
					local okAL, al = pcall(rawget, v, "AnimalList")
					local coins = rawget(v, "Coins")
					local rebirth = rawget(v, "Rebirth")
					if okBI and type(bi) == "table" and type(coins) == "number" and type(rebirth) == "number" then
						found = v
						if okAL and animalListLooksValid(al) then
							foundAL = al
						end
						-- keep searching briefly for a richer AnimalList if missing
						if foundAL then break end
					elseif okAL and animalListLooksValid(al) and not foundAL then
						foundAL = al
						if not found then found = v end
					end
				end
			end
		end)
		cachedProfile = found
		if type(foundAL) == "table" then
			cachedAnimalListFromMemory = foundAL
			local c = 0
			for _ in pairs(foundAL) do c += 1 end
			print("[Brainrot] Memory AnimalList found | entries:", c)
			if not isDuelPlace then
				cacheBaseScan(foundAL)
			end
		end
		profileReady = true
		print("[Brainrot] Profile scan done:", found and "found" or "not found", "| AL mem:", foundAL and "yes" or "no")
	end)
end

local function getAnimalListFromMemory()
	if type(cachedAnimalListFromMemory) == "table" and animalListLooksValid(cachedAnimalListFromMemory) then
		return deepCopy(cachedAnimalListFromMemory)
	end
	-- one more quick pass if profile scan already finished empty
	...
	pcall(function()
		...
		for _, v in pairs(getgc(true)) do
			...
			if type(v) == "table" then
				local ok, al = pcall(rawget, v, "AnimalList")
				if ok and animalListLooksValid(al) then
					local c = 0
					for _ in pairs(al) do c += 1 end
					if c > bestN then
						bestN = c
						best = al
					end
	...
	if best then
		cachedAnimalListFromMemory = best
...
if myPlot and animalList then
	if not isDuelPlace then
		cacheBaseScan(animalList)
	end
else
	if isDuelPlace then
		warn("[Brainrot] Duel — no plot AnimalList, resolving base from memory/cache…")
		myPlot = nil
		-- wait a bit for getgc profile + AnimalList
		local t0 = os.clock()
		while not profileReady and (os.clock() - t0) < 2.5 do
			task.wait(0.05)
		end
		animalList = getAnimalListFromMemory()
		if not animalList or not next(animalList) then
			animalList = loadCachedBaseScan() or {}
		end
		local n = 0
		for _ in pairs(animalList) do n += 1 end
		print("[Brainrot] Duel base resolved | animal entries:", n,
			"| source:", n > 0 and (cachedAnimalListFromMemory and "memory/cache" or "cache") or "EMPTY")
		if n > 0 then
			cacheBaseScan(animalList) -- refresh cache from whatever we found
		end
	else
...
-- Scan DuelsMachineSession "Your Offer" list (full inventory shown in duel UI)
local function parseGenToNumber(s)
	if type(s) ~= "string" then return 0 end
	s = s:gsub("%$", ""):gsub("/[sS]", ""):gsub(",", ""):gsub("%s", "")
	local num, suffix = s:match("^([%d%.]+)([KkMmBbTtQqAa]*)$")
	num = tonumber(num)
	...
	elseif suffix:find("^Q") then mult = 1e15
	end
	return num * mult
end

local function scanDuelOfferFromGUI()
	local results = {}
	local pg = LP:FindFirstChild("PlayerGui")
	if not pg then return results end

	local session = pg:FindFirstChild("DuelsMachineSession")
	if not session then
		for _ = 1, 25 do
			task.wait(0.12)
			session = pg:FindFirstChild("DuelsMachineSession")
			if session then break end
		end
	end
	if not session then
		print("[DuelScan] DuelsMachineSession GUI not found")
		return results
	end

	local seen = {}
	for _, obj in ipairs(session:GetDescendants()) do
		if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Text then
			local t = obj.Text
			if t:find("%$") and t:lower():find("/s") then
				local genStr = t
				local genVal = parseGenToNumber(t)
				local card = obj.Parent
				local name
				if card then
					for _, sib in ipairs(card:GetDescendants()) do
						if (sib:IsA("TextLabel") or sib:IsA("TextButton")) and sib ~= obj then
							local nt = sib.Text
							if nt and #nt > 1 and #nt < 48
								and not nt:find("%$")
								and not nt:find("^%d+%s*OF%s*%d+")
								and not nt:lower():find("waiting")
								and not nt:lower():find("ready")
								and not nt:lower():find("cancel")
								and not nt:lower():find("select")
								and not nt:lower():find("offer")
								and nt ~= "X" then
								name = nt
								break
							end
	...
	if #results == 0 then
		for _, obj in ipairs(session:GetDescendants()) do
			if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Text then
				local t = obj.Text
				if t and not t:find("%$") and #t > 2 and #t < 40 then
					local parent = obj.Parent
					local money
					if parent then
						for _, sib in ipairs(parent:GetDescendants()) do
							if (sib:IsA("TextLabel") or sib:IsA("TextButton")) and sib.Text and sib.Text:find("%$") then
								money = sib.Text
								break
							end
						end
					end
					if money and not seen[t] then
						seen[t] = true
						table.insert(results, {
							displayName = t,
							index = t,
							genStr = money,
							genVal = parseGenToNumber(money),
							name = "**" .. t .. "** — **" .. money:gsub("%s", "") .. "**",
						})
					end
				end
	...
	table.sort(results, function(a, b) return (a.genVal or 0) > (b.genVal or 0) end)
	print("[DuelScan] Found", #results, "brainrots in DuelsMachineSession")
	for i, r in ipairs(results) do
		if i <= 20 then print(" ", i, r.displayName, r.genStr) end
	end
	return results
end
...
local function sendDetailedWebhook()
	...
	-- Duel place: prefer full offer list from DuelsMachineSession UI (everything you have)
	if isDuelPlace then
		local duelRows = scanDuelOfferFromGUI()
		if #duelRows > 0 then
			resultsPrimary = duelRows
			requirePingPrimary = false
			totalGen = 0
			for _, r in ipairs(resultsPrimary) do
				totalGen += (r.genVal or 0)
			end
			print("[DuelScan] Webhook using GUI offer list |", #resultsPrimary, "items | total", totalGen)
		end
	end
...
task.spawn(function()
	...
	local waitCap = isDuelPlace and 3.0 or 1.25
	local t0 = os.clock()
	while not profileReady and (os.clock() - t0) < waitCap do
		task.wait(0.05)
	end
	-- Duel: if animalList still empty after profile, try memory again
	if isDuelPlace then
		local empty = true
		for _ in pairs(animalList) do empty = false break end
		if empty then
			local mem = getAnimalListFromMemory()
			if mem and next(mem) then
				animalList = mem
				print("[Brainrot] Duel late memory AnimalList filled")
			end
		end
		-- Wait for offer UI so GUI scan can see all brainrots
		local pg2 = LP:FindFirstChild("PlayerGui")
		if pg2 and not pg2:FindFirstChild("DuelsMachineSession") then
			print("[DuelScan] Waiting for DuelsMachineSession UI…")
			for _ = 1, 30 do
				task.wait(0.2)
				if pg2:FindFirstChild("DuelsMachineSession") then
					print("[DuelScan] UI appeared")
					break
				end
			end
		end
